<script>
import {getLogsLogin} from "@/pages/dashboard/helpers";
    ...
    getMonth(){
      ...
      getLogsLogin(true,true).then(
          (data) => {
            ...
            this.series = [
              {name: "Akses", data: totalData},
            ];
          });
    },

    getYear(){
      getLogsLogin(true).then(
          (data) => {
            const totalData = Array.from({ length: 12 }, () => 0);

            data.forEach(({ month, count }) => {
              totalData[month - 1] = count;
            });
  ...
  <div class="card">
    <div class="card-body">
      <div class="d-sm-flex flex-wrap">
        <h4 class="card-title mb-4">Infografis Akses Member Harian</h4>
