...
export default {
  ...
  created () {
    this.isAdmin = jwtDecode(JSON.parse(fastLocalStorage.getItem('user')).access_token).role === 1
  },
  mounted: function () {
  ...
  <div id="sidebar-menu">
    ...
    <ul id="side-menu" class="metismenu list-unstyled">
      <template v-for="item in menuItems">
        <li class="menu-title" v-if="item.isTitle && (!item.isAdmin || isAdmin)" :key="item.id">
          {{ item.label }}
        </li>
        <li v-if="!item.isTitle && !item.isLayout && (!item.isAdmin || isAdmin)" :key="item.id">
